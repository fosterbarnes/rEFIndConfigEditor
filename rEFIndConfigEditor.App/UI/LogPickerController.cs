...
internal static class LogPickerController
{
    ...
    public static async Task<IReadOnlyList<string>> PickItemsAsync(
        Window owner,
        ListPicker picker,
        ...
        IEnumerable<MenuEntry> menuEntries)
    {
        ...
        var logPath = await RefindLogPickerHelper.PickLogPathAsync(configFilePath).ConfigureAwait(true);
        ...
        if (items.Count == 0)
        {
            PlatformServices.Current.ShowWarning(AppBranding.DisplayName, picker.EmptyMessage);
            return [];
        }

        var volumes = await RefindLogImportWindow.PickVolumesAsync(
            owner,
            items,
            picker.DialogTitle,
            picker.Subtitle,
            pickPrompt: picker.PickPrompt).ConfigureAwait(true);
        return volumes;
    }
}
